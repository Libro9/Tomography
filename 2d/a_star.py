...
class Graph:
    def __init__(self, points, max_distance=0.5):
        self.graph = {}
        self.points = points
        self.max_distance = max_distance
        self._build_graph()
        self.neighbors = NearestNeighbors(n_neighbors=1)  # Nearest neighbor model for finding closest points
        self.neighbors.fit(self.points[:, 0:2])  # Fit only the first two dimensions (x, y)
    ...
    def heuristic(self, node, target):
        ...
        """
        return np.linalg.norm(self.points[node, :2] - self.points[target, :2])
    ...
    def plot(self, path, start, end, show_edges=False):
        ...
        path_points = self.points[[int(node) for node in path], 0:2]

        # Plot the points
        fig, ax = plt.subplots(figsize=(7, 7))
        ax.scatter(self.points[:, 0], self.points[:, 1], s=1, c='k', label='Grain Boundaries')

        # Plot the edges (connections between points)
        if show_edges:
            for node1, neighbors in self.graph.items():
                for node2, weight in neighbors.items():
                    point1 = self.points[int(node1), 0:2]
                    point2 = self.points[int(node2), 0:2]
                    ax.plot([point1[0], point2[0]], [point1[1], point2[1]], c='gray', lw=0.5)

        # Plot the path
        ax.plot(path_points[:, 0], path_points[:, 1], c='r', label='Path')

        # Highlight the start and end points
        ax.scatter(self.points[start, 0], self.points[start, 1], c='blue', label='Start')
        ax.scatter(self.points[end, 0], self.points[end, 1], c='green', label='End')

        # Set labels and title
        ax.set_xlabel('x')
        ax.set_ylabel('y')
        ax.legend()
        ax.set_title('Shortest Path along the Grain Boundaries in 2D')

        # Save and show the plot
        fig.savefig('2d/2D_Shortest_Path.png')
        return fig
...
def main(start_coords, end_coords, astar=True):
    ...
    points = np.genfromtxt('2d/points_1slice.csv', delimiter=',', skip_header=1)
    ...
    print(f"Start point: {points[start, 0:2]}")
    print(f"End point: {points[end, 0:2]}")
    ...
    if path != []:
        print(f"The shortest path from {points[start, 0:2]} to {points[end, 0:2]} has a length of {DISTANCE:.1f} and is shown in the figure.")
    else:
        print(f'No path found between {points[start, 0:2]} and {points[end, 0:2]}')
